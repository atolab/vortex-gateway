...
public class DdsEndpoint_DurabilityKind_Test
{
   ...
   private static final Logger logger = LoggerFactory
         .getLogger(DdsEndpoint_DurabilityKind_Test.class);
   ...
   public void init() throws UnsupportedEncodingException
   {
      ...
      final String fromURI = "dds:Chat_SimpleChatMessage_durability:0/Chat.SimpleChatMessage" + qos
            + filter + partition;
   ...
   public void testExample() throws IOException
   {
      ...
      // Check if Durability is well set in the Topic
      topic = ddsEndpoint.getTopic();
      topic.get_qos(holder);

      logger.info("Check if Durability is well set in the topic");
      // Check if Durability is well set in the Topic
      Assert.assertTrue("Durability is not well set in the topic ",
            holder.value.durability.kind == DDS.DurabilityQosPolicyKind.PERSISTENT_DURABILITY_QOS);

      // Check if Durability is well set in the Writer
      try
      {
         ...
         logger.info("Check if Durability is well set in the Writer");
         // Check if Durability is well set in the writer
         Assert.assertTrue(
               "Durability is not well set in the writer ",
               writerHolder.value.durability.kind == DDS.DurabilityQosPolicyKind.PERSISTENT_DURABILITY_QOS);
      ...
      // Check if Durability is well set in the Reader
      try
      {
         ...
         logger.info("Check if Durability is well set in the Reader");
         // Check if Durability is well set in the reader
         Assert.assertTrue(
               "Durability is not well set in the reader ",
               readerHolder.value.durability.kind == DDS.DurabilityQosPolicyKind.PERSISTENT_DURABILITY_QOS);
      ...
      // Check if DurabilityKind is well set in the DurabilityKind attribute
      try
      {
         logger.info("Check if DeadlinePeriod is well set in the ddsEnpoint attribute");
         // Check if DurabilityKind is well set in in the DurabilityKind attribute
         Assert.assertTrue(
               "DeadlinePeriod is not well set in in the ddsEnpoint attribute ",
               ddsEndpoint.getDurabilityKind() == DDS.DurabilityQosPolicyKind.PERSISTENT_DURABILITY_QOS);
