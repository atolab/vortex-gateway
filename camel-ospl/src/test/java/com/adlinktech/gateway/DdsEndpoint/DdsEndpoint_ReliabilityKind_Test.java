...
public class DdsEndpoint_ReliabilityKind_Test
{
   ...
   private static final Logger logger = LoggerFactory
         .getLogger(DdsEndpoint_ReliabilityKind_Test.class);
   ...
   public void init() throws UnsupportedEncodingException
   {
      ...
      final String fromURI = "dds:Chat_SimpleChatMessage_reliability:0/Chat.SimpleChatMessage"
   ...
   public void testExample() throws IOException
   {
      ...
      // Check if Reliability is well set in the Writer
      try
      {
         ...
         Assert.assertTrue(
               ...
               writerHolder.value.reliability.kind == DDS.ReliabilityQosPolicyKind.RELIABLE_RELIABILITY_QOS);
      ...
      // Check if Reliability is well set in the Reader
      try
      {
         ...
         Assert.assertTrue(
               ...
               readerHolder.value.reliability.kind == DDS.ReliabilityQosPolicyKind.RELIABLE_RELIABILITY_QOS);
      ...
      // Check if ReliabilityKind is well set in the ReliabilityKind attribute
      try
      {
         logger.info("Check if ReliabilityKind is well set in the ddsEnpoint attribute");
         // Check if ReliabilityKind is well set in in the ddsEnpoint attribute
         Assert.assertTrue(
               "ReliabilityKind is not well set in in the ddsEnpoint attribute ",
               ddsEndpoint.getReliabilityKind() == DDS.ReliabilityQosPolicyKind.RELIABLE_RELIABILITY_QOS);
