...
public class DdsEndpoint_HistoryKind_Test
{
   ...
   private static final Logger logger = LoggerFactory.getLogger(DdsEndpoint_HistoryKind_Test.class);
   ...
   public void init() throws UnsupportedEncodingException
   {
      ...
      final String fromURI = "dds:Chat_SimpleChatMessage_history:0/Chat.SimpleChatMessage" + qos
            + filter + partition;
   ...
   public void testExample() throws IOException
   {
      ...
      // Check if History is well set in the Topic
      topic = ddsEndpoint.getTopic();

      topic.get_qos(holder);

      logger.info("Check if HistoryKind is well set in the topic");
      // Check if History is well set in the Topic
      Assert.assertTrue("Reliability is not well set in the topic ",
            holder.value.history.kind == DDS.HistoryQosPolicyKind.KEEP_LAST_HISTORY_QOS);


      // Check if History is well set in the Writer
      try
      {
         ...
         logger.info("Check if HistoryKind is well set in the Writer");
         // Check if History is well set in the writer
         Assert.assertTrue("HistoryKind is not well set in the writer ",
               writerHolder.value.history.kind == DDS.HistoryQosPolicyKind.KEEP_LAST_HISTORY_QOS);
      ...
      // Check if History is well set in the Reader
      try
      {
         ...
         logger.info("Check if HistoryKind is well set in the Reader");
         // Check if History is well set in the reader
         Assert.assertTrue("HistoryKind is not well set in the reader ",
               readerHolder.value.history.kind == DDS.HistoryQosPolicyKind.KEEP_LAST_HISTORY_QOS);
      ...
      // Check if HistoryKind is well set in the HistoryKind attribute
      try
      {
         logger.info("Check if HistoryKind is well set in the ddsEnpoint attribute");
         // Check if HistoryKind is well set in in the HistoryKind attribute
         Assert.assertTrue("HistoryKind is not well set in in the ddsEnpoint attribute ",
               ddsEndpoint.getHistoryKind() == DDS.HistoryQosPolicyKind.KEEP_LAST_HISTORY_QOS);
