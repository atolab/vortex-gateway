...
public class DdsEndpoint_HistoryDepth_Test
{
   ...
   private static final Logger logger = LoggerFactory
         .getLogger(DdsEndpoint_HistoryDepth_Test.class);
   ...
   public void init() throws UnsupportedEncodingException
   {
      ...
      final String fromURI = "dds:Chat_SimpleChatMessage_HistoryDepth:0/Chat.SimpleChatMessage"
   ...
   public void testExample() throws IOException
   {
      ...
      // Check if HistoryDepth is well set in the Topic
      topic = ddsEndpoint.getTopic();

      topic.get_qos(holder);

      logger.info("Check if HistoryDepth is well set in the topic");
      // Check if HistoryDepth is well set in the Topic
      Assert.assertTrue("HistoryDepth is not well set in the topic ",
            holder.value.history.depth == 150);

      // Check if HistoryDepth is well set in the Writer
      try
      {
         ...
         logger.info("Check if HistoryDepth is well set in the Writer");
         // Check if HistoryDepth is well set in the writer
         Assert.assertTrue("HistoryDepth is not well set in the writer ",
               writerHolder.value.history.depth == 150);
      ...
      // Check if HistoryDepth is well set in the Reader
      try
      {
         ...
         logger.info("Check if HistoryDepth is well set in the Reader");
         // Check if HistoryDepth is well set in the reader
         Assert.assertTrue("HistoryDepth is not well set in the reader ",
               readerHolder.value.history.depth == 150);
      ...
      // Check if HistoryDepth is well set in the HistoryKind attribute
      try
      {
         logger.info("Check if HistoryDepth is well set in the ddsEnpoint attribute");
         // Check if HistoryDepth is well set in in the HistoryDepth attribute
         Assert.assertTrue("HistoryDepth is not well set in in the ddsEnpoint attribute ",
               ddsEndpoint.getHistoryDepth() == 150);
