...
public class DdsEndpoint_DurabilityServiceDepth_Test
{
   ...
   private static final Logger logger = LoggerFactory
         .getLogger(DdsEndpoint_DurabilityServiceDepth_Test.class);
   ...
   public void init() throws UnsupportedEncodingException
   {
      ...
      final String fromURI = "dds:Chat_SimpleChatMessage_DurabilityServiceDepth:0/Chat.SimpleChatMessage"
   ...
   public void testExample() throws IOException
   {
      ...
      // Check if Durability_Service depth is well set in the Wrier
      /*
       * try {
       ...
       * logger.info("Check if Durability_Service Depth is well set in the Writer");
       * // Check if Durability_Service depth is well set in the writer
       * Assert.assertTrue("Durability_service_depth is not well set in the writer ",
       * writerHolder.value.durability_service.history_depth==100);
       * } catch (Exception e) {
      ...
      // Check if Durability_Service Depth is well set in the Durability_Service Depth attribute
      try
      {
         logger.info("Check if Durability_Service Depth is well set in the ddsEnpoint attribute");
         // Check if Durability_Service Depth is well set in in the Durability_Service Depth
         // attribute
         Assert.assertTrue(
               "Durability_Service Depth is not well set in in the ddsEnpoint attribute ",
               ddsEndpoint.getDurabilityServiceDepth() == 100);
