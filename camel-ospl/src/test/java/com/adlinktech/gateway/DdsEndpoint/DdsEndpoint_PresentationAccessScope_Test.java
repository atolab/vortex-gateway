...
public class DdsEndpoint_PresentationAccessScope_Test
{
   ...
   private static final Logger logger = LoggerFactory
         .getLogger(DdsEndpoint_PresentationAccessScope_Test.class);
   ...
   public void init() throws UnsupportedEncodingException
   {
      ...
      final String fromURI = "dds:Chat_SimpleChatMessage_PresentationAccessScope:0/Chat.SimpleChatMessage"
   ...
   public void testExample() throws IOException
   {
      ...
      // Check if PresentationAccessScope is well set in the Publisher
      try
      {
         ...
         logger.info("Check if PresentationAccessScope is well set in the Publisher");
         // Check if PresentationAccessScope is well set in the publisher
         Assert.assertTrue(
               "PresentationAccessScope is not well set in the publisher ",
               publiserHolder.value.presentation.access_scope == DDS.PresentationQosPolicyAccessScopeKind.TOPIC_PRESENTATION_QOS);
      ...
      // Check if PresentationAccessScope is well set in the Subscriber
      try
      {
         ...
         logger.info("Check if PresentationAccessScope is well set in the Subscriber");
         // Check if PresentationAccessScope is well set in the Subscriber
         Assert.assertTrue(
               "PresentationAccessScope is not well set in the Subscriber ",
               subcriberHolder.value.presentation.access_scope == DDS.PresentationQosPolicyAccessScopeKind.TOPIC_PRESENTATION_QOS);
      ...
      // Check if PresentationAccessScope is well set in the PresentationAccessScope attribute
      try
      {
         logger.info("Check if PresentationAccessScope is well set in the ddsEnpoint attribute");
         // Check if PresentationAccessScope is well set in in the PresentationAccessScope attribute
         Assert.assertTrue(
               "PresentationAccessScope is not well set in in the ddsEnpoint attribute ",
               ddsEndpoint.getPresentationAccessScope() == DDS.PresentationQosPolicyAccessScopeKind.TOPIC_PRESENTATION_QOS);
